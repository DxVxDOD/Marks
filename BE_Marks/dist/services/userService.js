...
const getAllUsers = async () => {
	const { data: allUserData, error: allUserError } = await (0,
	promiseWrapper_1.wrapInPromise)(userModel_1.default.find({}));
...
const getUserById = async (id) => {
	const { data, error } = await (0, promiseWrapper_1.wrapInPromise)(
		userModel_1.default.findById((0, generalParsers_1.stringParser)(id)),
	);
	if (error || !data) {
		throw new Error(
...
const postNewUser = async (obj) => {
	const { data: allUsersData, error: allUsersError } = await (0,
	promiseWrapper_1.wrapInPromise)((0, exports.getAllUsers)());
	if (allUsersError || !allUsersData) {
		throw new Error("Error while fetching all users: " + allUsersError.message);
	}
	const { data: userData, error: userError } = await (0,
	promiseWrapper_1.wrapInPromise)(
		(0, userParser_1.newUserParser)(obj, allUsersData),
	);
	if (userError || !userData) {
		throw new Error("Error while parsing new user data: " + userError.message);
	}
	const { data: passwordHashed, error: passwordHashedError } = await (0,
	promiseWrapper_1.wrapInPromise)(
		bcrypt_1.default.hash((0, generalParsers_1.stringParser)(obj.password), 10),
	);
	...
	const { data: savedUser, error: savedUserError } = await (0,
