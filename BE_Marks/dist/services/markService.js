...
const getAllMarks = async () => {
	const { data: allMarks, error: allMarksError } = await (0,
	promiseWrapper_1.wrapInPromise)(
		markModel_1.default.find({}).populate("user", { username: 1, name: 1 }),
	);
...
const getMarkById = async (id) => {
	const { data, error } = await (0, promiseWrapper_1.wrapInPromise)(
		markModel_1.default
			.findById((0, generalParsers_1.stringParser)(id))
			.populate("user", {
				username: 1,
				name: 1,
			}),
...
const postNewMark = async (obj, user) => {
	const { data: markData, error: markError } = await (0,
	promiseWrapper_1.wrapInPromise)((0, markParser_1.newMarkParser)(obj));
	...
	const { data: savedMark, error: savedMarkError } = await (0,
	promiseWrapper_1.wrapInPromise)(mark.save());
	...
	const { data: updatedUser, error: updatedUserError } = await (0,
	promiseWrapper_1.wrapInPromise)(user.save());
...
const deleteMark = async (user, markId) => {
	const { data: mark, error: markError } = await (0,
	promiseWrapper_1.wrapInPromise)(
		markModel_1.default.findById((0, generalParsers_1.stringParser)(markId)),
	);
	if (!mark || markError) {
		throw new Error(
	...
	const { data: deleteData, error: deleteError } = await (0,
	promiseWrapper_1.wrapInPromise)(
		markModel_1.default.findByIdAndDelete(markId),
	);
...
const updateMark = async (mark, userId, markId) => {
	const { data: markData, error: markError } = await (0,
	promiseWrapper_1.wrapInPromise)((0, markParser_1.markParser)(mark));
	if (!markData || markError) {
		throw new Error(markError.message);
	}
	const { data: oldMark, error: oldMarkError } = await (0,
	promiseWrapper_1.wrapInPromise)(
		markModel_1.default.findById((0, generalParsers_1.stringParser)(markId)),
	);
	...
	const { data: user, error: userError } = await (0,
	promiseWrapper_1.wrapInPromise)(userModel_1.default.findById(userId));
	if (!user || userError) {
		throw new Error(
	...
	const { data: updatedMark, error: updatedMarkError } = await (0,
	promiseWrapper_1.wrapInPromise)(
		markModel_1.default.findByIdAndUpdate(oldMark.id, markData, { new: true }),
	);
