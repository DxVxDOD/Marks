...
const login = async (obj) => {
	...
	promiseWrapper_1.wrapInPromise)(userModel_1.default.findOne({ username }));
	if (userError || !userData) {
		throw new Error(
			"Error while fetching user by username: " + userError.message,
		);
	}
	const correctPassword = await (0, promiseWrapper_1.wrapInPromise)(
		bcrypt_1.default.compare(password, userData.password),
	);
