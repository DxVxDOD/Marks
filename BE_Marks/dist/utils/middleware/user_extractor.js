...
const userExtractor = async (req, res, next) => {
	...
	const { data: token, error: tokenError } = await (0,
	promiseWrapper_1.wrapInPromise)(
		(0, token_extractor_1.tokenExtractor)(req, res, next),
	);
	if (!token || tokenError) {
		return res.status(401).json({ error: "{token} " + tokenError });
	}
	const { data: decodedToken, error: decodedTokenError } = await (0,
	promiseWrapper_1.wrapInPromise)(jsonwebtoken_1.default.verify(token, SECRET));
	if (!decodedToken || decodedTokenError) {
		return res
			.status(401)
			.json({ error: "{decodedToken} " + decodedTokenError });
	}
	const { data: verifiedToken, error: verifiedTokenError } = await (0,
	promiseWrapper_1.wrapInPromise)(
		(0, generalParsers_1.jwtPayloadParser)(decodedToken),
	);
	...
	promiseWrapper_1.wrapInPromise)(
		userModel_1.default.findById(verifiedToken.id),
	);
	if (userError || !user) {
