...
router.get("/", async (_req, res) => {
  const allUsers = await (0, promiseWrapper_1.wrapInPromise)(
    (0, userService_1.getAllUsers)(),
  );
...
router.post("/", async (req, res) => {
  const { data: newUserData, error: newUserError } = await (0,
  promiseWrapper_1.wrapInPromise)((0, userService_1.postNewUser)(req.body));
