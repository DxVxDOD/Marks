...
router.get("/", async (_req, res) => {
	...
	promiseWrapper_1.wrapInPromise)((0, markService_1.getAllMarks)());
...
router.get("/:id", async (req, res) => {
	const mark = await (0, promiseWrapper_1.wrapInPromise)(
		(0, markService_1.getMarkById)(req.params.id),
	);
...
router.post("/", user_extractor_1.userExtractor, async (req, res) => {
	...
	const { data: newMark, error: newMarkError } = await (0,
	promiseWrapper_1.wrapInPromise)(
		(0, markService_1.postNewMark)(req.body, user),
	);
...
router.put("/:id", user_extractor_1.userExtractor, async (req, res) => {
	const { data, error } = await (0, promiseWrapper_1.wrapInPromise)(
		(0, markService_1.updateMark)(req.body, res.locals.user.id, req.params.id),
	);
	if (!data || error) {
...
router.put("/:id/like", async (req, res) => {
	...
	const { data, error } = await (0, promiseWrapper_1.wrapInPromise)(
		(0, markService_1.updateMark)(req.body, userId, req.params.id),
	);
	if (!data || error) {
		res.status(400).json({ error: error.message });
	}
...
router.delete("/:id", user_extractor_1.userExtractor, async (req, res) => {
	const { data, error } = await (0, promiseWrapper_1.wrapInPromise)(
		(0, markService_1.deleteMark)(res.locals.user, req.params.id),
	);
	if (!data || error) {
